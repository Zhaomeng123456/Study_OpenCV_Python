...
**当前进度**：Day1 ~ Day8 已实现（第一周完成，第二周全局二值化已开始）。
...
│   ├── images/                  # 放置测试图片
│   ├── output/                  # 脚本输出目录
│   └── day8_threshold.py        # 全局二值化 + 滑动条调参
├── Day9/ ~ Day28/               # 后续每日练习（待实现）
```
...
将测试图片（如 `cat1.jpg`）放入 `Day8/images/` 目录。

### 2. 运行全局二值化脚本

```bash
python Day8/day8_threshold.py
```

脚本将图片转为灰度图，通过滑动条实时调节全局阈值，按 **q** 键退出并保存，输出：

- `day8_binary.jpg` — 当前阈值下的二值化结果
